...
export async function speechToText(audioBlob: Blob): Promise<string> {
  ...
  const { data } = await http.post<{ text: string }>("/api/voice/stt", form);
  return data.text;
}

/** Convert text to speech via ElevenLabs. Returns an audio URL. */
export async function textToSpeech(text: string): Promise<string> {
  const { data } = await http.post("/api/voice/tts", null, {
    params: { text },
    responseType: "blob",
  });
