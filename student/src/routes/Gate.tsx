  ...
  return (
    <div className="relative flex min-h-screen flex-col px-5 py-6 sm:px-10 sm:py-8">
        ...
        <div className="mt-10 grid max-w-[820px] gap-3 sm:grid-cols-2">
          {DOORS.map(({ role, label, line }) => (
            <button
              key={role}
              onClick={() => choose(role)}
              className="card group flex flex-col px-5 py-6 text-left transition-colors duration-150 hover:bg-paper-2"
            >
      ...
      <footer className="relative flex flex-wrap items-center gap-x-4 gap-y-2 border-t border-line pt-4">
        <span className="label-cap">You can change this later</span>
        <button
          onClick={() => nav("/welcome")}
